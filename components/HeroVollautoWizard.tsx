...
const PERSON_OPTIONS = [
  { value: 'weniger-20', label: 'Weniger als 20' },
  { value: '20-60', label: '20–60' },
  { value: 'mehr-60', label: 'Mehr als 60' }
];
...
export default function HeroVollautoWizard() {
  ...
  const updateData = (field: keyof WizardData, value: string) => {
    ...
    if (field === 'personen' && currentStep === 1) {
      setTimeout(() => setCurrentStep(2), 300);
    } else if (field === 'wunsch' && currentStep === 2) {
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      if (!data.personen) newErrors.personen = 'Bitte wählen Sie eine Option';
    } else if (step === 2) {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      source_page: 'vollautomaten',
      einsatzort: '',
      personen_oder_mitarbeiter: data.personen,
      tassen_pro_tag: '',
      service_wichtigkeit: '',
      vertragslaufzeit: '',
    ...
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).dataLayer) {
        (window as any).dataLayer.push({
          event: 'lead_submit',
          source_section: 'vollautomaten_hero_wizard',
  ...
  if (isSuccess) {
    return (
      <div className="bg-white rounded-2xl shadow-lg p-8 text-center max-w-md mx-auto">
        ...
        <h3 className="text-2xl font-bold text-logo-brown mb-2">Vielen Dank!</h3>
        <p className="text-logo-brown/70">
          Wir melden uns i.d.R. innerhalb von 1–3 Tagen.
        </p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8 max-w-lg mx-auto">
      ...
      <div className="mb-8">
        ...
        <div className="flex justify-between text-sm text-logo-brown/70">
          <span className={currentStep === 1 ? 'font-semibold text-logo-brown' : ''}>
            Versorgung
          </span>
          <span className={currentStep === 2 ? 'font-semibold text-logo-brown' : ''}>
            Wunsch
          </span>
          <span className={currentStep === 3 ? 'font-semibold text-logo-brown' : ''}>
            Kontakt
          </span>
        </div>
      </div>
      ...
      {currentStep === 1 && (
        <fieldset role="group">
          <legend className="lato-bold text-xl text-logo-brown mb-6">
            Wie viele Personen sollen mit Kaffee versorgt werden?
          </legend>
          <div className="space-y-3">
            {PERSON_OPTIONS.map((option) => (
              <button
                key={option.value}
                type="button"
                onClick={() => updateData('personen', option.value)}
          ...
          {errors.personen && (
            <p className="text-red-600 text-sm mt-2">{errors.personen}</p>
          )}
        </fieldset>
      )}
          ...
          <div className="grid grid-cols-2 gap-3">
            {WUNSCH_OPTIONS.map((option) => (
              <button
                ...
                className={`p-4 rounded-lg border-2 text-sm lato-bold transition-all focus-visible:outline focus-visible:outline-2 focus-visible:outline-logo-blue ${
      ...
      {currentStep === 3 && (
        <fieldset role="group">
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="wizard-name" className="block text-sm lato-bold mb-1 text-logo-brown">
                Name *
              </label>
              <input
                id="wizard-name"
            ...
            <div>
              <label htmlFor="wizard-email" className="block text-sm lato-bold mb-1 text-logo-brown">
                E-Mail *
              </label>
              <input
                id="wizard-email"
            ...
            <div>
              <label htmlFor="wizard-telefon" className="block text-sm lato-bold mb-1 text-logo-brown">
                Telefonnummer *
              </label>
              <input
                id="wizard-telefon"
            ...
            <div>
              <label htmlFor="wizard-firma" className="block text-sm lato-bold mb-1 text-logo-brown">
                Firma (optional)
              </label>
              <input
                id="wizard-firma"
      ...
      <div className="flex justify-between items-center mt-8">
        ...
        {currentStep === 3 && (
          <div className="text-right">
            ...
            >
              {isSubmitting ? 'Wird gesendet...' : 'Jetzt passende Lösung finden'}
            </button>
